...
export default class Database {
  constructor() {
    ...
    this.db.version(1).stores({
      ...
      project:
        "++id, name, managedBy, hasMembers, deadline, description, status",
      task:
        "++id, name, doneBy, assignedBy, underProject, tag, deadline, description, status", //tag: b, t, i, d
    });
    this.db.open();
  }
  ...
  async getProject(projectNameInput) {
    ...
    await this.db.project
      ...
      .catch((error) =>
        console.log("Error performing get Project operation:🥺 ", error)
      );
  ...
  async userManages(usernameInput, projectNameInput) {
    ...
    let result = await this.db.users
      ...
      .catch((err) =>
        console.log(
          "Adding a project to a list of project managed by a user failed:🥺 ",
          err
        )
      );
  }
  ...
  async completeProject(projectNameInput) {
    this.db.project.where("name").equals(projectNameInput).modify({
      status: 1,
    });
  }
  ...
  async addProjectToUser(usernameInput, projectNameInput) {
    ...
    await this.db.users
      ...
      .then((ismemberOf) => {
        this.db.users.where("username").equals(usernameInput).modify({
          memberOf: ismemberOf,
        });
      });
  }
