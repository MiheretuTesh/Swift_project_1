...
if (projectCards) {
  document.addEventListener("click", (e) => {
    ...
    if (
      ...
      target.parentElement.classList.contains("boards")
    ) {
    ...
    DB.createProject(
      ...
      DB.getProjects().then((projects) => {
        ...
        userNames.forEach((user) =>
          DB.addProjectToUser(user, projectName.value)
        );
        location.reload();
      });
    });
...
if (addTaskBtn) {
  DB.getProject(currentProject)
    .then((project) => {
      return [project.managedBy, ...project.hasMembers];
    })
    .then((projects) =>
      addTaskBtn.addEventListener("click", (e) => ui.addTask(projects))
    );
}
...
if (taskAddingModal) {
  cardModalSave.addEventListener("click", (e) => {
    e.preventDefault();
    const userListSelector = taskAddingModal
  ...
  document.addEventListener("DOMContentLoaded", (e) => {
    e.preventDefault();
    DB.getTasks(currentProject).then((tasks) => {
...
if (wrapper) {
  ...
  wrapper.addEventListener("dragend", (e) => {
    ...
    const tagName = draggedTo.children
      .item(0)
      .querySelector("p")
      .textContent.trim();
...
if (registerForm) {
  registerForm.addEventListener("submit", (e) => {
    e.preventDefault();
    DB.getUser(uname_register.value).then((exists) => {
      if (exists) {
...
if (addTaskBtn) {
  wrapper.addEventListener("click", (e) => {
    if (
      ...
      e.target.classList.contains("inner-list") ||
      e.target.parentElement.classList.contains("inner-list")
    ) {
      let target;
      ...
      DB.getTask(taskName).then((task) => ui.taskInfo(task));
    }
  });
}
